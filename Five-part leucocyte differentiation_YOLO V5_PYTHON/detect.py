...
def run(weights='yolov5s.pt',  # model.pt path(s)
        ...
        savexyxy = False # True 保存box 的 左上角，右小角绝对坐标；否则保存 归一化的 xywh
        ):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:
                        ...
                        if savexyxy:
                            ...
                            xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh(xcenter, ycenter, width, height)
...
def parse_opt():
    ...
    parser.add_argument('--savexyxy', action='store_true', help='save xyxy or xywh')
