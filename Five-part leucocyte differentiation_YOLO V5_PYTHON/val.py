...
from utils.plots import output_to_target, plot_images, plot_val_study, plot_cls_p_R_f1
...
def run(data,
    ...
    s = ('%20s' + '%11s' * 7) % ('Class', 'Images', 'Labels', 'P', 'R', 'f1', 'mAP@.5', 'mAP@.5:.95')
    ...
    print(pf % ('all', seen, nt.sum(), mp, mr,mf, map50, map))
    ...
    # 各类别指标曲线绘制
    allclassnamelist = np.array(list(names.values()))
    plot_cls_p_R_f1(allclassnamelist, p, r, f1,mp,mr,mf, save_dir = save_dir)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='./data/StainWBCdetect.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default='D:\duan\yolov5\yolov5-master_09172021\yolov5-master\\runs\\train\StainWBCdetect\weights\\best.pt', help='model.pt path(s)')
