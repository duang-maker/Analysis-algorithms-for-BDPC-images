...
def plot_cls_p_R_f1(name,p,r,f1, mp,mr,mf, save_dir=Path('')):
    ...
    plt.plot(x, p, marker='o', color="blue", label="Precision", linewidth=1.5)
    plt.plot(x, r, marker='o', color="green", label="Recall", linewidth=1.5)
    plt.plot(x, f1, marker='o', color="red", label="F1 Score", linewidth=1.5)
    plt.plot(x, np.ones(cn)*mp,  "b--", label="Mean Precision %.3f" % mp , linewidth=1,alpha=0.8)
    plt.plot(x, np.ones(cn)*mr, "g--", label= "Mean Recall    %.3f" % mr, linewidth=1, alpha=0.8)
    plt.plot(x, np.ones(cn)*mf, "r--", label= "Mean F1 score  %.3f" % mf, linewidth=1, alpha=0.8,)

    group_labels = list(x)  # x轴刻度的标识
    plt.xticks(x, group_labels, fontsize=12, fontweight='bold')  # 默认字体大小为10
    # plt.xticks(x,  fontsize=12, fontweight='bold')
    plt.yticks(fontsize=12, fontweight='bold')
    # plt.title("example", fontsize=12, fontweight='bold')  # 默认字体大小为12
    plt.xlabel("Class Name", fontsize=13, fontweight='bold')
    plt.ylabel("Classification Result", fontsize=13, fontweight='bold')
    ...
    plt.savefig(save_dir / 'Classification result.jpg')
    matplotlib.use('Agg')
    # plt.show()
    print('')
    plt.close()
